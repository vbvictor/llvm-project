...
public:
  shared_ptr() = default;
  template <typename U> shared_ptr(const shared_ptr<U> &) {}
};
...
void f_smart_pointer_cast()
{
  ...
  // CHECK-MESSAGES: :[[@LINE-1]]:3: warning: use auto when initializing with a template cast to avoid duplicating the type name
  // CHECK-FIXES: auto sp2 = std::dynamic_pointer_cast<const B>(base);

  // MyPtr is in the SmartPointers list, should be transformed
  MyPtr<A> mbase;
  MyPtr<B> mp1 = my_pointer_cast<B>(mbase);
  // CHECK-MESSAGES: :[[@LINE-1]]:3: warning: use auto when initializing with a template cast to avoid duplicating the type name
  // CHECK-FIXES: auto mp1 = my_pointer_cast<B>(mbase);
  ...
  boost::shared_ptr<A> bbase;
  boost::shared_ptr<B> bsp = boost::dynamic_pointer_cast<B>(bbase);
